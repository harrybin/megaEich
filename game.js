...
const MVP_NAMES = ['Alex', 'Sam', 'Jordan', 'Casey', 'Morgan', 'Riley', 'Taylor', 'Drew'];
...
const LEVEL_DEFS = [
    // Level 1
    {
        acorns: 6, golden: 1, powerups: 0, mvpCount: 1,
        platforms: [
    ...
    // Level 2
    {
        acorns: 9, golden: 2, powerups: 1, mvpCount: 2,
        platforms: [
    ...
    // Level 3
    {
        acorns: 12, golden: 3, powerups: 2, mvpCount: 3,
        platforms: [
...
function loadLevel(num) {
    ...
    for (let i = 0; i < def.acorns; i++) {
        const p = shuffled[idx % shuffled.length]; idx++;
        collectibles.push({
            type: 'acorn',
...
function drawTree(x, baseY, scale) {
    ...
    ctx.beginPath();
    ctx.moveTo(0, -118); ctx.lineTo(-38, -55); ctx.lineTo(38, -55);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = '#2E7D32';
    ctx.beginPath();
    ctx.moveTo(0, -96); ctx.lineTo(-48, -28); ctx.lineTo(48, -28);
    ctx.closePath(); ctx.fill();
    ctx.restore();
}
...
function drawSquirrel(x, y, facing, walkFrame, speedBoost) {
    ...
    ctx.fillStyle = '#C4622D';
    ctx.beginPath();
    ctx.moveTo(-6, 2 + bob); ctx.lineTo(-10, -7 + bob); ctx.lineTo(-1, -1 + bob);
    ctx.fill();
    ctx.beginPath();
    ctx.moveTo(8, 2 + bob); ctx.lineTo(13, -7 + bob); ctx.lineTo(4, -1 + bob);
    ctx.fill();
    ctx.fillStyle = '#FFAABB';
    ctx.beginPath();
    ctx.moveTo(-5, 1 + bob); ctx.lineTo(-8, -4 + bob); ctx.lineTo(-1, -1 + bob);
    ctx.fill();
    ctx.beginPath();
    ctx.moveTo(7, 1 + bob); ctx.lineTo(11, -4 + bob); ctx.lineTo(4, -1 + bob);
    ctx.fill();
...
function drawMVP(x, y, name, bounceTimer, met) {
    ...
    ctx.lineWidth = 7;
    ctx.lineCap = 'round';
    ctx.beginPath(); ctx.moveTo(-5, 30); ctx.lineTo(-7, 50); ctx.stroke();
    ctx.beginPath(); ctx.moveTo( 5, 30); ctx.lineTo( 7, 50); ctx.stroke();
    ...
    ctx.lineWidth = 6;
    ctx.beginPath(); ctx.moveTo(-14, 12); ctx.lineTo(-22, 28); ctx.stroke();
    ctx.beginPath(); ctx.moveTo( 14, 12); ctx.lineTo( 22, 28); ctx.stroke();

    // Hands
    ctx.beginPath(); ctx.arc(-22, 28, 5, 0, Math.PI * 2);
    ...
    ctx.fillStyle = skinColor; ctx.fill();
    ctx.strokeStyle = '#D09080'; ctx.lineWidth = 1; ctx.stroke();
    ...
    ctx.strokeStyle = '#884444'; ctx.lineWidth = 1.5; ctx.stroke();
...
function drawAcorn(x, y) {
    ...
    ctx.strokeStyle = '#5C3317'; ctx.lineWidth = 2; ctx.stroke();
    // Body
    ctx.beginPath();
    ctx.ellipse(0, 4, 7, 9, 0, 0, Math.PI * 2);
    ctx.fillStyle = '#8B4513'; ctx.fill();
    ctx.strokeStyle = '#5C2510'; ctx.lineWidth = 1; ctx.stroke();
    // Shine
    ctx.beginPath();
...
function drawNotification() {
    ...
    roundRect(W / 2 - tw / 2, 86 - ySlide, tw, 28, 6, 'rgba(0,0,0,0.75)', '#FFD700');
    ctx.fillStyle = '#FFD700';
...
function drawLevelCompleteScreen() {
    ...
    roundRect(W / 2 - 230, H / 2 - 122, 460, 244, 14, 'rgba(0,20,60,0.92)', '#FFD700');
...
function drawGameOverScreen() {
    ...
    roundRect(W / 2 - 210, H / 2 - 112, 420, 224, 14, 'rgba(60,0,0,0.92)', '#EF5350');
...
function update() {
    ...
    if (state === 'GAME_OVER') {
        if (keys['Space'] || keys['Enter']) { level = 1; score = 0; lives = 3; startGame(); }
        return;
    }
    if (state === 'LEVEL_COMPLETE') {
        if (keys['Space'] || keys['Enter']) { level++; loadLevel(level); state = 'PLAYING'; }
        return;
    }
...
function render() {
    ...
    // Squirrel
    drawSquirrel(player.x - cameraX, player.y, player.facing, player.walkFrame, player.speedBoost);
